...
from pushbase.colors import Rgb
...
class PPMeter(ControlSurfaceComponent, APCMessenger):
  ...
  def __init__(self, track, parent, top = MASTER_SCALE_MAX, bottom = MASTER_SCALE_MIN, *a, **k):
  ...
  def set_light(self):
    for x in range(self.led_count):
      for y in range(self.columns):
        ...
        if x < self.led_index:
          # self.parent.log_message("Turning PPM Light On")
          button.set_light(COLORS[x])
        else:
          button.set_light("PPM.Off") 
          # self.parent.log_message("Turning PPM Light Off")
