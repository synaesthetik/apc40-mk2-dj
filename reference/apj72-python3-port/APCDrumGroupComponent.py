...
class APCDrumGroupComponent(DrumGroupComponent, APCMessenger):
  ...
  def _update_control_from_script(self):
    ...
    if self.drum_matrix:
      #for button, _ in ifilter(first, self.drum_matrix.iterbuttons()):
      for button in self.drum_matrix:
        if button._control_element:
          button._control_element.set_channel(PAD_FEEDBACK_CHANNEL)
          button._control_element.set_enabled(takeover_drums)
